...
Metadata_Add_String (struct List *list, uuid_t name, const char *string)
{
    ...
    char * type = UUID_Get_NameByUUID(name, UUID_TYPE_NTLV_NAME);
    rzb_log(LOG_NOTICE,"%s - %s - %s",__func__, type, string);
...
Metadata_Add_IPv4 (struct List *list, uuid_t name, const uint8_t *addr)
{
    ...
    char * type = UUID_Get_NameByUUID(name, UUID_TYPE_NTLV_NAME);
    rzb_log(LOG_NOTICE,"%s - %s - %s",__func__, type, addr);
...
Metadata_Add_Port (struct List *list, uuid_t name, const uint16_t port)
{
    ...
    char * type = UUID_Get_NameByUUID(name, UUID_TYPE_NTLV_NAME);
	rzb_log(LOG_NOTICE,"%s - %s - %u",__func__, type, port);
...
SO_PUBLIC bool 
Metadata_Get_Filename (struct List *list, uint32_t *len, const char **name)
{
    uuid_t uuidName;
    if (!UUID_Get_UUID(NTLV_NAME_FILENAME, UUID_TYPE_NTLV_NAME, uuidName))
    {
    ...
    return Metadata_Get_String(list, uuidName, len, name);
}
